...
async function init() {
    ...
    // Load installed items (prefer native storage, fallback to localStorage)
    loadInstalledItems();
    ...
    // On startup, scan the install base path for already-installed apps.
    // This helps when localStorage was reset during an update.
    await preScanInstalledItemsFromDisk();

    await fetchManifest();
    setupEventListeners();
}

// Startup scan: infer installed apps from disk layout
// Expected layout: <installBasePath>\<ProductName>\<Version>\...
async function preScanInstalledItemsFromDisk() {
    if (!installBasePath) return;
    if (typeof nativeListDirectory !== 'function') return;

    try {
        const products = await listDirectory(installBasePath);
        const productDirs = products.filter(e => e && e.isDirectory);
        ...
        for (const product of productDirs) {
            ...
            const productPath = installBasePath + '\\' + productName;
            const versions = await listDirectory(productPath);
            const versionDirs = versions.filter(e => e && e.isDirectory);
            ...
            for (const ver of versionDirs) {
                ...
                if (installedItems[key]?.installed === true) continue;

                const installPath = productPath + '\\' + version;
                installedItems[key] = {
                    installed: true,
                    installPath,
                    installDate: null,
        ...
        console.warn('Failed to pre-scan installed items from disk:', e);
    }
}
...
async function determineInstallPath() {
    ...
    // Try native settings storage (survives app updates)
    try {
        if (typeof nativeLoadSettings === 'function') {
            ...
            if (result) {
                ...
                const settingsPath = settings.installLocation || settings.downloadLocation;
                if (settingsPath) {
                    console.log('Using native settings path:', settingsPath);
                    installBasePath = settingsPath;
                    return;
                }
            }
...
// Best-effort rehydration: if installed-items state is missing, infer it from disk.
// This helps recover after an update that reset localStorage.
async function rehydrateInstalledItemsFromDisk() {
    if (!installBasePath) return;
    if (typeof nativePathExists !== 'function') return;
    ...
    for (const item of availableDownloads) {
        ...
        const expectedInstallPath = installBasePath + '\\' + item.name + '\\' + item.version;
        const hasFolder = await pathExists(expectedInstallPath);
        if (!hasFolder) continue;

        let finalInstallPath = expectedInstallPath;
        if (item.entry) {
            finalInstallPath = await resolveInstallPathForEntry(expectedInstallPath, item.entry);
            const hasEntry = await pathExists(finalInstallPath + '\\' + item.entry);
...
// Load installed items (prefer native file storage if available)
function loadInstalledItems() {
    // 1) Prefer native file-backed storage (stable across app updates)
    try {
        if (typeof nativeLoadInstalledItems === 'function') {
            const result = nativeLoadInstalledItems();
            if (result) {
                installedItems = JSON.parse(result);
                return;
            }
        }
    } catch (e) {
    ...
    try {
        ...
        console.error('Failed to load installed items from localStorage:', e);
        installedItems = {};
    }

    // 3) Best-effort migration to native storage so future updates keep state
    try {
        if (loadedFromLocal && typeof nativeSaveInstalledItems === 'function') {
            nativeSaveInstalledItems(JSON.stringify(installedItems));
        }
    } catch (e) {
        console.warn('Could not migrate installed items to native storage:', e);
    }
}

// Save installed items
function saveInstalledItems() {
    try {
        localStorage.setItem('atlas_installed_items', JSON.stringify(installedItems));
        if (typeof nativeSaveInstalledItems === 'function') {
            nativeSaveInstalledItems(JSON.stringify(installedItems));
        }
    } catch (e) {
...
async function processManifest(manifest) {
    ...
    // If state was lost (eg after update), infer installed items from disk.
    await rehydrateInstalledItemsFromDisk();
...
function getInstallStatus(item) {
    // Special case: AtlasCore is always "installed" (it's the running app)
    if (item.name === ATLASCORE_PRODUCT_NAME && appInfo && appInfo.version) {
        const cmp = compareVersions(item.version, appInfo.version);
        if (cmp > 0) {
...
function renderActions(item, hasMultipleVersions) {
    // Special case: AtlasCore (self-update)
    if (item.name === ATLASCORE_PRODUCT_NAME && appInfo && appInfo.version) {
        let html = '';
        const cmp = compareVersions(item.version, appInfo.version);
        ...
        if (cmp > 0) {
            // Newer version available
            html += `<button class="btn-primary" onclick="startSelfUpdate()">Update to v${escapeHtml(item.version)}</button>`;
        } else if (cmp === 0) {
            // Current version
            html += `<span class="current-version-label">Current Version</span>`;
        } else {
            // Older version in manifest (shouldn't normally happen)
            html += `<span class="current-version-label">Older Version</span>`;
        }
