(function() {
    ...
    function urlVariants(url) {
        ...
        const out = [];
        if (u) out.push(u);
        // Some embedded pages may redirect between ".html" and non-".html" forms.
        if (u && u.endsWith('.html')) {
            out.push(u.slice(0, -'.html'.length));
        }
        return out;
    }

    function isSafePrefixMatch(target, prefix) {
        if (!target || !prefix) return false;
        if (!target.startsWith(prefix)) return false;
        if (target.length === prefix.length) return true;
        const next = target.charAt(prefix.length);
        // Only treat as a match if the next character is a path/query/hash boundary.
    ...
    window.sidebarSetActiveByUrl = function sidebarSetActiveByUrl(url) {
        ...
        // Prefer exact matches; otherwise choose the most specific safe prefix match.
        let bestTab = null;
        let bestLen = -1;

        for (const item of navItems) {
            const variants = urlVariants(item.dataset.url);
            if (!variants.length) continue;

            for (const itemUrl of variants) {
                ...
                // Safe prefix match (handles redirects like "/embedded" -> "/embedded/" but avoids "/embedded-mail").
                if (isSafePrefixMatch(target, itemUrl) && itemUrl.length > bestLen) {
                    bestTab = item.dataset.tab;
